 ...
 * <p>
 * <p>
 * A {@link kamon.metric.Histogram Histogram} for the defining method with the name {@code coolName}  will be created which uses the
 * annotated method's return as its operationName.
 */
@Documented
...
public @interface Histogram {
    ...
    /**
     * The highest operationName to be tracked by the histogram. Must be a positive integer that is >= (2 * lowestDiscernibleValue).
     * Must not be larger than (Long.MAX_VALUE/2).
     */
    long highestTrackableValue() default 3600000000000L;

    /**
     * The number of significant decimal digits to which the histogram will maintain operationName resolution and separation.
